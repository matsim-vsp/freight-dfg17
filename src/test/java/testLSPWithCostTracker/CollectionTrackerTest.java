		...
		for(EventHandler handler : linearTracker.getEventHandlers()) {
			if(handler instanceof TourStartHandler) {
				...
				for(ScheduledTour scheduledTour : carrier.getSelectedPlan().getScheduledTours()) {
					scheduledCosts += scheduledTour.getVehicle().getVehicleType().getVehicleCostInformation().getFix();
				...
				for(ScheduledTour scheduledTour: carrier.getSelectedPlan().getScheduledTours()) {
					...
					for(TourElement element : tour.getTourElements()) {
						if(element instanceof ServiceActivity){
							...
							scheduledCosts += activity.getService().getServiceDuration() * scheduledTour.getVehicle().getVehicleType().getVehicleCostInformation().getPerTimeUnit();
							totalScheduledCosts += scheduledCosts;
			...
			if(handler instanceof DistanceAndTimeHandler) {
				...
				for(ScheduledTour scheduledTour : carrier.getSelectedPlan().getScheduledTours()) {
					...
					for(TourElement element : tour.getTourElements() ) {
						if(element instanceof Leg) {
							Leg leg = (Leg) element;
							scheduledTimeCosts += leg.getExpectedTransportTime() * scheduledTour.getVehicle().getVehicleType().getVehicleCostInformation().getPerTimeUnit();
						}
					}
				}
				...
				for(ScheduledTour scheduledTour : carrier.getSelectedPlan().getScheduledTours()) {
					scheduledDistanceCosts += network.getLinks().get(scheduledTour.getTour().getEndLinkId()).getLength() * scheduledTour.getVehicle().getVehicleType().getVehicleCostInformation().getPerDistanceUnit();
					for(TourElement element : scheduledTour.getTour().getTourElements()) {
						...
						if(element instanceof Leg) {
							...
							for(Id<Link> linkId: linkRoute.getLinkIds()) {
								scheduledDistanceCosts  += network.getLinks().get(linkId).getLength() * scheduledTour.getVehicle().getVehicleType().getVehicleCostInformation().getPerDistanceUnit();
							}
						}
						if(element instanceof ServiceActivity) {
							ServiceActivity activity = (ServiceActivity) element;
							scheduledDistanceCosts  += network.getLinks().get(activity.getLocation()).getLength() * scheduledTour.getVehicle().getVehicleType().getVehicleCostInformation().getPerDistanceUnit();
						}
					}
				}
