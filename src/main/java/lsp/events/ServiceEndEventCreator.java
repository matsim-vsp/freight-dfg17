...
public final class ServiceEndEventCreator implements EventCreator{
		...
		if(event instanceof ActivityEndEvent){
			...
			if(endEvent.getActType() == "service") {
				...
				if(element instanceof ServiceActivity) {
					...
					return new ServiceEndEvent(endEvent, carrier.getId(), driverId, serviceActivity.getService(), event.getTime(), scheduledTour.getVehicle());
				}
			}	
		}
		return null;
